...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="home-hero"><div className="hero-grid" aria-hidden="true"/><div className="hero-glow" aria-hidden="true"/><div className="container-page hero-layout">
      ...
      <motion.div className="hero-art" initial={reduce ? false : {opacity:0, x:35}} animate={{opacity:1,x:0}} transition={{duration:.8,delay:.15}}><div className="hero-logo-card"><img src="/brand/first-class-express-logo.png" fetchPriority="high" alt="1st Class Express — Committed to Deliver a Quality Service" width="560" height="608"/></div><div className="freight-line"><Truck/><span/></div><p>Sydney <i/> Brisbane <i/> Melbourne <i/> Adelaide</p></motion.div>
    ...
    <section className="section fleet-showcase"><div className="container-page"><div className="fleet-heading"><SectionHeading eyebrow="The right vehicle" title="A fleet that scales with the freight" intro="Our vehicles range from 1 tonne vans to pallet trucks, sizing up to semi-trailers and B-double trailers."/><ButtonLink to="/our-fleet" variant="secondary">View the Fleet</ButtonLink></div><div className="fleet-strip">{fleet.slice(0,5).map(({title,use,icon:Icon},i)=><article key={title}><span>0{i+1}</span><Icon/><h3>{title}</h3><p>{use}</p></article>)}</div></div></section>
