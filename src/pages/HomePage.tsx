...
export default function HomePage() {
  ...
  return <>
    ...
    <AnimatedLogoIntro/>
    <section className="home-hero"><div className="hero-media" aria-hidden="true"><img src="/images/generated/hero-interstate-truck.jpg" alt="" /></div><div className="hero-media-overlay" aria-hidden="true"/><div className="hero-grid" aria-hidden="true"/><div className="hero-glow" aria-hidden="true"/><div className="container-page hero-layout">
    ...
    <section className="section fleet-showcase"><div className="container-page"><div className="fleet-heading"><SectionHeading eyebrow="The right vehicle" title="A fleet that scales with the freight" intro="Our vehicles range from 1 tonne vans to pallet trucks, sizing up to semi-trailers and B-double trailers."/><ButtonLink to="/our-fleet" variant="secondary">View the Fleet</ButtonLink></div><figure className="fleet-feature-image"><img src="/images/generated/fleet-lineup.jpg" loading="lazy" width="1920" height="1080" alt="Unbranded delivery van, rigid truck, prime mover and semi-trailer arranged in an Australian freight yard"/><figcaption><span>One network</span><strong>Multiple vehicle classes</strong></figcaption></figure><div className="fleet-strip">{fleet.slice(0,5).map(({title,use,icon:Icon},i)=><article key={title}><span>0{i+1}</span><Icon/><h3>{title}</h3><p>{use}</p></article>)}</div></div></section>
