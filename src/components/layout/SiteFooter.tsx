...
export function SiteFooter() {
  return <footer className="site-footer"><div className="container-page footer-grid">
    ...
    <div><h2>Explore</h2><Link to="/">Home</Link><Link to="/about">About Us</Link><Link to="/services">Services</Link><Link to="/fleet">Our Fleet</Link><Link to="/service-areas">Service Areas</Link><Link to="/careers">Careers</Link><Link to="/contact">Contact</Link><Link to="/quote">Request a Quote</Link></div>
    <div><h2>Services</h2>{services.slice(0,6).map(service=><Link key={service.id} to={`/services#${service.id}`}>{service.title}</Link>)}</div>
    <div><h2>Contact</h2><a href={phoneHref(company.phonePrimary)}><Phone size={16}/>{company.phonePrimary}</a><a href={`mailto:${company.email}`}><Mail size={16}/>{company.email}</a><span><MapPin size={16}/>Australia-wide service</span></div>
