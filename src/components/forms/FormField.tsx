...
export function FormField({ label, error, registration, type = 'text', placeholder, className = '', required = true }: { label: string; error?: FieldError; registration: UseFormRegisterReturn; type?: string; placeholder?: string; className?: string; required?: boolean }) {
  ...
  return <label className={`form-field ${className}`} htmlFor={id}><span>{label}{required && <em className="required-mark" aria-hidden="true"> *</em>}</span><input id={id} type={type} placeholder={placeholder} autoComplete={autocomplete[id] ?? 'off'} required={required} aria-required={required} {...registration} aria-invalid={!!error} aria-describedby={error ? `${id}-error` : undefined}/>{error && <small id={`${id}-error`} role="alert">{error.message}</small>}</label>
}
