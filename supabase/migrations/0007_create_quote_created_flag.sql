-- create_quote() now reports whether this call actually inserted a new quote
-- (`created: true`) or returned an existing one via the idempotency key
-- (`created: false`). The API uses this to send the acknowledgement / internal
-- emails only on a genuine first submission — an idempotent retry must not
-- re-send them.
...
begin
  if v_key is not null then
    ...
    if v_id is not null then
      return public.quote_detail_json(v_id) || jsonb_build_object('created', false);
    end if;
  end if;
  ...
  return public.quote_detail_json(v_id) || jsonb_build_object('created', true);
end $$;
